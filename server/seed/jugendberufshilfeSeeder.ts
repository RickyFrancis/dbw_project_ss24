...
async function saveDataToFile(data: any) {
  ...
  const filename = `jugendberufshilfen_${currentDate}.json`;
...
async function fetchDataAndUpsert() {
  try {
    const response = await axios.get(process.env.JUGENDBERUFHILFEN_API_URL);
    saveDataToFile(response.data);
    const features = response.data.features;

    await prisma.$transaction(
      features.map((feature) => {
        return prisma.jugendberufshilfe.upsert({
          where: {
    ...
    console.log('Jugendberufshilfe Data upserted successfully.');
  } catch (error) {
